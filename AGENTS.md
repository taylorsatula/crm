...
- **Canonical Ticket Closeout**: `TicketCloseoutService` is the only terminal API workflow. It treats current line items as quoted baseline, applies only typed exceptions, requires structured escalation packets with an owned action for damage, safety, customer concern/dispute, billing uncertainty, and return work, blocks invoicing until `invoice.resolve_billing_hold` confirms the existing reconciled total, preserves quoted/final snapshots, writes provenance-bearing profile facts, creates exact tickets or flexible plans plus actions, and publishes `TicketCompleted` only after the transaction commits. `ticket.close` is not an API action.
