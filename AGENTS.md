...
- **Know Thy Self**: I (Codex) have a tendency to make up new endpoints or change existing patterns instead of looking at what's already there. Always look at existing code before making assumptions.
...
- **Canonical Ticket Closeout**: `TicketCloseoutService` is the only terminal API workflow. It records actual duration on the ticket, persists one thin closeout aggregate (duration, scope/result status, customer response, next-service disposition, technician summary), and closes the ticket in a single transaction. `ticket.close` is not an API action. Durable customer/property facts are not part of the closeout payload — they live as customer-scoped notes/attributes written via `records_tool` during the MIRA debrief.
- **Customer Portrait Loop**: `_ticket_packet` (`api/data.py`) resurfaces the accumulated customer lens on every packet — `customer_notes` (customer-scoped notes; ticket packets previously carried only ticket-scoped `notes`), `recent_closeouts` (last 5 via `TicketCloseoutService.list_for_customer`), and `attributes`. This powers the debrief baseline, the `/data/today` board, and ticket list packets from one shared function. Bucket discipline: visit events → ticket notes; durable state → customer notes/attributes.
- **Square History Import**: `SquareImportService` is the only boundary allowed to create immutable historical tickets and Square sales without schedule validation or operational lifecycle events. `square_import_links` owns retry idempotency; booking/order couples arrive pre-matched from CRM Mira (chronological per-customer sequence pairing, or `manual_review` from the interactive review queue). `_ensure_service` collapses Square's price-change variation recreations onto one canonical service per normalized name.
- `deploy/migrations/20260717_square_history.sql` and `deploy/migrations/20260721_structured_ticket_closeout.sql` upgrade existing durable CRM databases; fresh installs receive the same tables and ticket columns from `schema.sql`. Compose's one-shot `migrations` service records/applies the structured-closeout migration before `crm` starts and fails on a stale workspace base schema or partial closeout schema state.
