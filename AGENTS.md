...
- `deploy/migrations/*.sql` upgrade existing durable CRM databases; fresh installs receive the same tables from `schema.sql`. Compose's one-shot `migrations` service applies every unapplied migration in lexicographic order before `crm` starts (tracked in `schema_migrations`, each file self-contained with its own transaction) and fails on a stale workspace base schema.
