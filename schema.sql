...
-- ticket_closeouts
-- -----------------------------------------------------------------------------
-- Structured closeout record for completed service appointments.
-- -----------------------------------------------------------------------------
...
-- quotes
-- -----------------------------------------------------------------------------
-- Price proposals sent to a customer before a job is booked.
-- Lifecycle: draft → sent → accepted | rejected | expired | archived
-- When accepted, optionally links to a created ticket.
-- Soft delete. RLS filters to current workspace AND not deleted.
-- -----------------------------------------------------------------------------
CREATE TABLE quotes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    -- Link to ticket created on acceptance
    created_ticket_id UUID REFERENCES tickets(id) ON DELETE SET NULL,

    -- Lead origin (if created from lead conversion / pipeline)
...
-- quote_line_items
-- -----------------------------------------------------------------------------
-- Service lines that compose a quote. Reference optional service catalog entry.
-- Prices stored in cents. Soft delete. Soft-deleted with the parent quote,
-- but the table supports independent soft delete for draft editing.
-- -----------------------------------------------------------------------------
